...
KB.http.get(url).success(function(response){isOpen=!0;create(response,getWidth(size),overlayClickDestroy)})},close:function(){destroy()},isOpen:function(){return isOpen},replace:function(url){KB.http.get(url).success(function(response){replace(response)})},getForm:getForm,submitForm:submitForm}}());KB.on('dom.ready',function(){function tooltip(mytarget){if(mytarget.__target&&document.contains(mytarget.__target))return!0;mytarget.__destroy=function(){if(mytarget.__onChild&&document.contains(mytarget.__onChild)){return!1}
if(mytarget.__target){mytarget.__target.parentNode&&mytarget.__target.parentNode.removeChild(mytarget.__target);mytarget.__target=null}}
mytarget.__onTooltip=!1;mytarget.__onChild=!1;mytarget.__mouseOnTooltip=function(){mytarget.__onTooltip=!0}
var containerElement=document.createElement("div");containerElement.__opener=mytarget;containerElement.addEventListener("mouseleave",mytarget.__destroy,!1);containerElement.addEventListener("mouseenter",mytarget.__mouseOnTooltip,!1);containerElement.classList.add('tooltip-container');mytarget.__target=containerElement;var containers=document.getElementsByClassName("tooltip-container");if(containers.length){$.each(containers,function(idx,container){if(mytarget.__target&&mytarget.__target===container){return!0}
if(!(container&&container.contains(mytarget))){container.__opener.__destroy()}else{container.__opener.__onChild=containerElement}})}
create(mytarget);document.body.appendChild(containerElement);mytarget.addEventListener('mouseout',function(e){if(!mytarget.__onTooltip)return!1;setTimeout(function(){mytarget.__destroy()},500)})}
...
function render(element,html){var containerElement=element.__target;if(!containerElement)return!1;containerElement.innerHTML=html;var elementRect=element.getBoundingClientRect();var top=elementRect.top+window.scrollY+elementRect.height;containerElement.style.top=top+"px";if(elementRect.left>(window.innerWidth-600)){var right=window.innerWidth-elementRect.right-window.scrollX;containerElement.style.right=right+"px"}else{var left=elementRect.left+window.scrollX;containerElement.style.left=left+"px"}}
document.body.addEventListener('mouseover',function(e){if(e.target.classList.contains('tooltip')){tooltip(e.target)}
if(e.target.classList.contains('fa')&&e.target.parentNode.classList.contains('tooltip')){tooltip(e.target.parentNode)}});document.body.addEventListener('click',function(event){var c=document.getElementsByClassName("tooltip-container");if(c.length){$.each(document.getElementsByClassName("tooltip-container"),function(idx,el){if(!(el&&el.contains(event.target))){if(el&&el.__opener){el.__opener&&el.__opener.__destroy()}}})}})});KB.utils.formatDuration=function(d){if(d>=86400){return Math.round(d/86400)+"d"}else if(d>=3600){return Math.round(d/3600)+"h"}else if(d>=60){return Math.round(d/60)+"m"}
...
hideActionMenu()}
function onItemChange(event){var selectedItems=document.querySelectorAll("input[data-list-item=selectable]:checked");if(selectedItems.length>0){showActionMenu()}}
...
function onActionClick(event){event.preventDefault();var selectedItems=document.querySelectorAll("input[data-list-item=selectable]:checked");var taskIDs=[];var s={};for(var i=0;i<selectedItems.length;i++){taskIDs.push(selectedItems[i].value);var n=selectedItems[i].name.replace("[","").replace("]","");if(n==="tasks"){n="task_ids"}
if(n==="subtasks"){n="subtask_ids"}
if(typeof s[n]==="undefined"){s[n]=[]}
s[n].push(selectedItems[i].value)}
var link=event.target.href;var first=event.target.href.indexOf("?")===-1?!0:!1;for(item in s){link+=(first?"?":"&")+item+"="+s[item].join(",");first=!1}
KB.modal.open(link,"medium",!0)}
